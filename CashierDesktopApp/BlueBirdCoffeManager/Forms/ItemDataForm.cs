...
namespace BlueBirdCoffeManager.Forms
{
    public partial class ItemDataForm : Form
    {
        ...
        List<OrderDetailViewModel> currentOrder;
        public ItemDataForm(string searchItem, Guid? categoryId, Panel oDataPanel, OrderViewModel? editOrder)
        {
            InitializeComponent();
        ...
        private void ItemDataForm_Load(object sender, EventArgs e)
        {
            try
            {
                if(_editOrder == null)
                {
                    currentOrder = Sessions.Order.CurrentOrder.OrderDetail;
                }
                else
                {
                    currentOrder = _editOrder.OrderDetails;
                }
                ...
                for (int i = 0; i < _matchItem.Count; i++)
                {
                    ...
                    {
                        var curItem = currentOrder.FirstOrDefault(f => f.ItemId == item.Id);
                        if (curItem != null)
                        {
                        ...
                        else
                        {
                            ...
                            currentOrder.Add(orderDetailViewModel);
                        }
            ...
            {
                MessageBox.Show("Có lỗi xảy ra, vui lòng tải lại trang!");
            }
        }
    }
}
